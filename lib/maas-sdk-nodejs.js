...
MiraclClient.prototype.getAuthorizationRequestUrl = function(session) {
  var params = { response_type: 'code', client_id: this.options.client_id, redirect_uri: this.options.redirect_uri };
...
MiraclClient.prototype._getTokenAndUserinfo = function(code, grantType, session, callback) {
  var oauth2 = new OAuth2(this.options.client_id,  this.options.secret, '', this.config.authorizationURL, this.config.tokenURL);
  ...
  if(grantType === 'refresh') {
    ...
    oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: this.options.redirect_uri }, function(error, accessToken, refreshToken, params) {
      if(error) {
        return callback(new MiraclError(messages.access_token_failed, error.data));
      }
