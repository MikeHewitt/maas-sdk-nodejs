...
MiraclClient.prototype.isAuthorized = function(session) {
  if(session[SESSION_MIRACL_TOKEN_KEY]) return session[SESSION_MIRACL_TOKEN_KEY];
  else return false;
}
...
MiraclClient.prototype._getTokenAndUserinfo = function(code, grantType, session, cb) {
  ...
  if(grantType === 'refresh'){
    ...
    oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code',
        ...
        if (!idToken) { return cb(new MiraclError("ID Token not present in token response"))};
 ...
 * @private
 * @param {String} mesage
 * @param  {Error} err Original error text
